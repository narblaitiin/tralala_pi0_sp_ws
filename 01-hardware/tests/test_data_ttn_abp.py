...
def getPayloadMockSensor():
    ...
    return encodePayload(press_val,temp_val,hum_val,lux_val,uvs_val,gas_val)

def encodePayload(pressure,temperature,humidity,luxmen,uvs,gas):
    # encode float as int
    ...
    # gas (needs 2 bytes)
    data[11] = (gas_val >> 8) & 0xff
    data[12] = gas_val & 0xff
...
# init TTN application and device (node 221 - ABP authentification)
devaddr = [0x00, 0x85, 0xBB, 0xCA]
